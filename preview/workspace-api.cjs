...
function workspaceApi({artifacts,port}){
 ...
 async function handle(req,res,url){
  ...
  try{
   ...
   if(route==='context'&&req.method==='GET')send({schemaVersion:1,sites:store.list('sites').map(({blocks,...s})=>({...s,cells:blocks.length})),drafts:store.list('drafts').map(d=>({id:d.id,name:d.plan.name,version:d.version,valid:d.valid,candidateHash:d.candidate.hash,parentHash:d.parentHash,project:d.project,siteId:d.siteId,author:d.author||null,createdAt:d.createdAt})),revisions:store.list('revisions'),jobs:store.list('jobs').map(j=>({id:j.id,state:j.state,kind:j.kind,artifactHash:j.artifactHash,world:j.world,createdAt:j.createdAt})),limits:{buildCells:10000,surveyVolume:262144,queue:4},models:'External agent workflow; no provider calls'});
