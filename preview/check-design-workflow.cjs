...
for(const width of [390,320]){await p.setViewportSize({width,height:1000});await p.evaluate(()=>scrollTo(0,0));assert.equal(await p.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);if(out&&width===390)await p.screenshot({path:path.join(out,'workspace-orange-phone.png')});}
