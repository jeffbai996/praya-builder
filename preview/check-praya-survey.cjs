...
async function main(){
 ...
 try{
  ...
  assert.equal(await page.locator('#drawing-empty').isVisible(),false);assert.equal(await page.locator('#new-proposals').isDisabled(),true);assert.equal(await page.locator('#current-design-name').textContent(),'Choose a design');assert.match(await page.locator('#site-summary').textContent(),/27 × 15 plot/);
  ...
  await page.reload();await page.waitForFunction(id=>document.querySelector('#site-select').value===id&&document.body.dataset.busy==='false',id,{timeout:90000});assert.equal(await page.locator('#current-design-name').textContent(),'Choose a design');
