...
async function main(){
 const {placementReadiness:ready,compatibleSites,placementFit:fit,placementErrorMessage}=await import('data:text/javascript;base64,'+fs.readFileSync(__dirname+'/placement-readiness.js').toString('base64'));
 ...
 const draft={id:'draft',valid:true,candidate,surveyHash:'survey'};
 ...
 assert.deepEqual(compatibleSites([site,{...site,id:'original',worldId:'original'},{...site,id:'outside',plot:{min:[-21,0,0],max:[0,1,1]}}],bridge),[site]);
