...
class DesignService{
 ...
 async evaluate(plan,siteId,transform){
  ...
  const access=checkAccess(site,artifact,transform),diagnostics=diagnose(artifact,site,transform,{assessment,access});
  return {candidate:artifact,valid:!diagnostics.some(d=>d.severity==='error'),diagnosticsVersion:DIAGNOSTICS_VERSION,diagnostics,assessment,access};
 }
 createDraft(input){return this.serialized(async()=>{
  ...
  const history=[{plan,candidateHash:result.candidate.hash,valid:result.valid,diagnosticsVersion:result.diagnosticsVersion,diagnostics:result.diagnostics,access:result.access,author}];
 ...
 editDraft(id,input){return this.serialized(async()=>{
  ...
  catch(error){result={candidate:draft.candidate,valid:false,diagnosticsVersion:DIAGNOSTICS_VERSION,diagnostics:[{rule:'compile.invalid-plan',version:1,severity:'error',component:input.componentId||null,at:null,message:error.message,hint:'Correct the plan and compile again; the last valid geometry is retained.'}],assessment:null,access:null};}
  const history=draft.history.slice(0,draft.cursor+1);history.push({plan,candidateHash:result.candidate.hash,valid:result.valid,diagnosticsVersion:result.diagnosticsVersion,diagnostics:result.diagnostics,access:result.access,author});
  if(history.length>50)history.shift();
  return this.store.update('drafts',id,input.expectedVersion,{...draft,author,plan,...result,history,cursor:history.length-1});
 });}
 history(id,input){return this.serialized(async()=>{
  ...
  return this.store.update('drafts',id,input.expectedVersion,{...draft,plan:entry.plan,candidate,valid:entry.valid,diagnostics:entry.diagnostics,diagnosticsVersion:entry.diagnosticsVersion||null,access:entry.access||null,author:entry.author||null,assessment,cursor});
 });}
