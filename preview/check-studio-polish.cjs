...
// Read-only against a running preview; it never saves, edits or creates drafts.
...
async function main(){
 ...
 try{
  ...
  if(versions.length>=2){
   ...
   assert.deepEqual(await page.evaluate(()=>studioStatus().camera),camera,'camera retained across compare');
  ...
  if(sibling){const before=await page.evaluate(()=>studioStatus().camera);await page.selectOption('#draft-select',sibling.id);await settled();const after=await page.evaluate(()=>studioStatus().camera);const a=await(await fetch(base+'/api/workspace/drafts/'+withVersions.id)).json(),b=await(await fetch(base+'/api/workspace/drafts/'+sibling.id)).json();if(a.candidate.dimensions.x===b.candidate.dimensions.x&&a.candidate.dimensions.z===b.candidate.dimensions.z)assert.deepEqual(after,before,'camera retained across designs');}
