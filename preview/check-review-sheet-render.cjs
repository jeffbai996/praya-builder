...
async function main(){
  ...
  try{
    ...
    const result=await page.evaluate(async()=>{
      ...
      const dark=await renderer.renderReviewView({...input,view:planView,theme:'dark'});const darkPixel=await new Promise(resolve=>{const image=new Image();image.onload=()=>{const c=document.createElement('canvas');c.width=960;c.height=760;const ctx=c.getContext('2d');ctx.drawImage(image,0,0);resolve([...ctx.getImageData(0,0,1,1).data]);};image.src=dark.png;});const assertFont=document.fonts.check('600 24px Urbanist');
      ...
      return {darkPixel,assertFont,darkHash:dark.pngHash,exteriors,elevations,plan:{png:plan.png,hash:plan.hash,pngHash:plan.pngHash,ceiling:plan.ceiling,kind:plan.kind,projection:plan.projection,markers:plan.markers,markerItems:plan.markerItems,diagnosticCount:plan.diagnosticCount,labels:plan.labels},repeatHash:repeat.pngHash,full:{pngHash:full.pngHash,markers:full.markers,diagnosticCount:full.diagnosticCount},dimensions,invalid};
    });
    assert.ok(result.darkPixel.slice(0,3).every(n=>n<60));assert.ok(result.assertFont);assert.notEqual(result.darkHash,result.plan.pngHash);
