...
async function action(work){if(busy)return;busy=true;document.body.dataset.busy='true';try{await work();}catch(error){if(error.status===409&&draft)conflict(error);else status(error.message);}finally{busy=false;document.body.dataset.busy='false';}}
// A 409 means the draft moved under us. Keep the camera and cutaway; let the user reload the draft on their terms.
function conflict(error){const box=$('studio-status');box.textContent='This design changed elsewhere; your view is kept. ';const actions=document.createElement('span');actions.className='conflict-actions';const reload=document.createElement('button');reload.type='button';reload.textContent='Reload design';reload.onclick=()=>action(async()=>{await refresh();await renderDraft(await api('drafts/'+draft.id));status('Design reloaded.');});const detail=document.createElement('button');detail.type='button';detail.textContent='Details';detail.onclick=()=>{status(error.message);};actions.append(reload,detail);box.append(actions);}
...
async function renderDraft(next,{frame=false}={}){
 ...
 review(draft);support();references();
...
$('catalogue-draft').onclick=()=>action(async()=>{const d=await api('drafts',{...placement(),catalogue:$('catalogue-select').value});await refresh();await renderDraft(d,{frame:true});mode('design');});
$('import-plan').onclick=()=>action(async()=>{const file=$('plan-file').files[0];if(!file)throw Error('Choose a plan JSON file');const d=await api('drafts',{...placement(),plan:JSON.parse(await file.text())});await refresh();await renderDraft(d,{frame:true});mode('design');});
